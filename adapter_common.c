...
/*
 * When DEMO_STANDALONE is defined the code skips waiting for a serial
 * "start" packet and initialises USB immediately.  Use this build when
 * flashing a bare Pro Micro for verification without the rest of the
 * GIMX adapter hardware (e.g. to show the device enumerates as a USB
 * joystick to a customer).  The normal firmware requires the external
 * controller to send the packet, so it will hang in the loop otherwise.
 */
#define DEMO_STANDALONE
...
void SetupHardware(void) {
    ...
    /*
     * Normally the firmware waits for a "start" packet over the serial
     * link before initialising the USB stack; this packet comes from the
     * external MCU in the full GIMX adapter.  When building a standalone
     * demo that is flashed directly to a Pro Micro without the external
     * controller, there is nobody to send that packet, so the device would
     * hang in the loop and never enumerate.
     *
     * Define DEMO_STANDALONE to bypass the wait and force the adapter to
     * start the USB subsystem immediately.  This makes the board appear as
     * a joystick for testing/verification purposes.  Remove the define or
     * rebuild without it for normal operation.
     */
#ifdef DEMO_STANDALONE
    started = 1;
#else
    while (!started) {}
#endif
